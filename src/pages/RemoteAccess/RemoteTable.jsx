...
import { deleteRemote } from "../../service/RemoteApi";
...
export default function RemoteTable({ data, onEdit, refreshData, selectedCompanyId }) {
    ...
    const [visibleColumns, setVisibleColumns] = useState({
        ...
        ipAddress: true,
        user: true,
        password: false // Nueva columna oculta por defecto
    });
    const [dropdownOpen, setDropdownOpen] = useState(false);
    const [showPasswords, setShowPasswords] = useState({}); // Estado para controlar qué contraseñas se muestran
    ...
    const { mutate: deleteMutate } = useMutation({
        ...
        onSuccess: () => {
            ToastSuccess("Registro eliminado con éxito");
            refreshData();
        },
        onError: (err) => ToastError(err.message || "Error al eliminar el registro"),
    ...
    // Toggle para mostrar/ocultar contraseña específica
    ...
    const columns = [
        ...
        { id: "terminal", label: "Terminal", visible: visibleColumns.terminal },
        { id: "software", label: "Software", visible: visibleColumns.software },
        { id: "ipAddress", label: "Dirección IP", visible: visibleColumns.ipAddress },
        { id: "user", label: "Usuario", visible: visibleColumns.user },
        { id: "password", label: "Contraseña", visible: visibleColumns.password }
    ];

    // Count visible columns for responsive design
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {filteredData.length > 0 ? (
                            filteredData.map((remote) => (
                                <tr
                                    key={remote.id}
                                    className="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"
                                >
                                    {visibleColumns.customer && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{remote.customer}</td>
                                    )}
                                    {visibleColumns.terminal && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{remote.terminal}</td>
                                    )}
                                    {visibleColumns.software && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{remote.software}</td>
                                    )}
                                    {visibleColumns.ipAddress && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100 font-mono">{remote.ipAddress}</td>
                                    )}
                                    {visibleColumns.user && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{remote.user}</td>
                                    )}
                                    {visibleColumns.password && (
                                        <td className="px-4 py-3 text-gray-900 dark:text-gray-100">
                                            <div className="flex items-center">
                                                <span className="font-mono">
                                                    {showPasswords[remote.id] ? remote.password : '••••••••'}
                                                </span>
                                                <button
                                                    onClick={() => toggleShowPassword(remote.id)}
                                                    className="ml-2 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
                                                    title={showPasswords[remote.id] ? "Ocultar" : "Mostrar"}
                                                >
                                                    {showPasswords[remote.id] ? (
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex justify-end space-x-2">
                                            ...
                                            <button
                                                onClick={() => handleDelete(remote.id, remote.customer)}
                            ...
                            <tr>
                                ...
                                >
                                    {searchTerm ? "No se encontraron resultados para su búsqueda" : "No hay registros disponibles"}
                                </td>
                            </tr>
                        )}
                    </tbody>
            ...
            <div className="md:hidden space-y-4 p-4">
                {filteredData.map(remote => (
                    <div key={remote.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border border-gray-200 dark:border-gray-700">
                        {visibleColumns.customer && (
                            <div className="mb-2">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Cliente</p>
                                <p className="text-base font-medium text-gray-900 dark:text-white">{remote.customer}</p>
                            </div>
                        )}
                        {visibleColumns.terminal && (
                            <div className="mb-2">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Terminal</p>
                                <p className="text-base font-medium text-gray-900 dark:text-white">{remote.terminal}</p>
                            </div>
                        )}
                        {visibleColumns.software && (
                            <div className="mb-2">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Software</p>
                                <p className="text-base font-medium text-gray-900 dark:text-white">{remote.software}</p>
                            </div>
                        )}
                        {visibleColumns.ipAddress && (
                            <div className="mb-2">
                                <p className="text-sm text-gray-500 dark:text-gray-400">IP</p>
                                <p className="text-base font-medium text-gray-900 dark:text-white font-mono">{remote.ipAddress}</p>
                            </div>
                        )}
                        {visibleColumns.user && (
                            <div className="mb-2">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Usuario</p>
                                <p className="text-base font-medium text-gray-900 dark:text-white">{remote.user}</p>
                            </div>
                        )}
                        {visibleColumns.password && (
                            <div className="mb-2">
