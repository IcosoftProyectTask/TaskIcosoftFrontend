...
function RealtimeChart({ data, width, height }) {
  ...
  useEffect(() => {
    // Obtener el valor actual y el valor anterior para calcular la diferencia
    ...
    // Formatear el valor actual en colones costarricenses
    chartValue.current.innerHTML = new Intl.NumberFormat("es-CR", {
      style: "currency",
      currency: "CRC",
    }).format(currentValue);
  
    // Cambiar color de desviación basado en el valor de `diff`
    if (diff < 0) {
    ...
    // Mostrar la desviación en porcentaje
